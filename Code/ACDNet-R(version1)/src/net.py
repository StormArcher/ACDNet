...
# =>PAM
class INAG(nn.Module):
    def __init__(self):
        super(INAG, self).__init__()
        kernel_size = 3
        # =>PAM
        self.CASA = CASA()
...
# Decoder(FPN-PAM)
class DecoderINAG(nn.Module):
    def __init__(self):
        super(DecoderINAG, self).__init__()
        # PAM
        self.INAG4 = INAG()
        # one part of FPN
        self.fpn3 = FPN()
    ...
    def forward(self, out2s, out3s, out4s, out5s):
        # =>PAM
        iam4 = self.INAG4(out3s, out4s, out5s) # PAM
...
class ACDNet(nn.Module): 
    def __init__(self, cfg):
        ...
        self.bkbone_D = ResNet_D()  # Encoder-D
        self.bkbone_3 = ResNet_3()  # DCS
        ...
        #  For ACG weights
        self.squeezeGS = Gate(2048*2)
        self.squeezeGD = Gate(2048*2)
        self.squeezeGS3 = Gate(2048 * 2)
        self.squeezeGD3 = Gate(2048 * 2)
        self.squeezeGS4 = Gate(2048 * 2)
        self.squeezeGD4 = Gate(2048 * 2)  
        # Decoder
        self.decoderS = DecoderINAG()  # Decoder-R(FPN-PAM)
        self.decoderD = DecoderINAG()  # Decoder-D(FPN-PAM)
        self.decoderF = DecoderINAG()  # Decoder-F(FPN-PAM)
    ...
    def forward(self, x, depth, shape=None):  # RGB-D: x is RGB img, depth is D img
        ...
        out1S, out2S, out3S, out4S, out5S = self.bkbone(x)  # ResNet()
        
        # =>bkbone for Depth
        depth = F.relu(self.bn1(self.conv1(depth)), inplace=True)
        # side outputs of Encoder-D: Sd1, Sd2, Sd3, Sd4, Sd5 
        out1D, out2D, out3D, out4D, out5D = self.bkbone_D(depth)  # ResNet()

        # =>squeeze for side outputs
        # channel from 2048, 1024, 512, 256 to 64
        # R2, R3, R3, R4, R5, activation function is ReLU
        out2s, out3s, out4s, out5s = self.squeeze2s(out2S), self.squeeze3s(out3S), self.squeeze4s(out4S), self.squeeze5s(out5S)
        # R2', R3', R4', activation function is Sigmoid
        out2sf, out3sf, out4sf = self.squeeze2sf(out2S), self.squeeze3sf(out3S), self.squeeze4sf(out4S)
        out2sf, out3sf, out4sf = torch.sigmoid(out2sf), torch.sigmoid(out3sf), torch.sigmoid(out4sf)

        # D2, D3, D3, D4, D5, activation function is ReLU
        out2d, out3d, out4d, out5d = self.squeeze2d(out2D), self.squeeze3d(out3D), self.squeeze4d(out4D), self.squeeze5d(out5D)
        # D2', D3', D4', activation function is Sigmoid
        out2df, out3df, out4df = self.squeeze2df(out2D), self.squeeze3df(out3D), self.squeeze4df(out4D)
        out2df, out3df, out4df = torch.sigmoid(out2df), torch.sigmoid(out3df), torch.sigmoid(out4df)

        # =>Decoder-R(FPN-PAM)
        outS = self.decoderS(out2s, out3s, out4s, out5s)
        # =>Decoder-D(FPN-PAM)
        outD = self.decoderD(out2d, out3d, out4d, out5d)
        
        # =>For ACG: cat(Sr5, Sd5)
        SD = torch.cat((out5S, out5D), 1)
        # generate weight
        GS = self.squeezeGS(SD)  # G1r
        GD = self.squeezeGD(SD)  # G1d
        GS3 = self.squeezeGS3(SD)  # G2r
        GD3 = self.squeezeGD3(SD)  # G2d
        GS4 = self.squeezeGS4(SD)  # G3r
        GD4 = self.squeezeGD4(SD)  # G3d

        # => DCS
        # inputs: outS, outD, out2sf, out2df, out3sf, out3df, out4sf, out4df
        #    are  R,    D,    R2',    D2',    R3',    D3',    R4',    D4'
        # outputs of DCS: out2sf, out2df, out3, out4, out5
        out2sf, out2df, out3, out4, out5 = self.bkbone_3(outS, outD, out2sf, out2df, out3sf, out3df, out4sf, out4df, GS, GD, GS3, GD3, GS4, GD4)
        # inputs: cat(out2sf, out2df)， out3, out4, out5 are outputs of DCS
        # outputs: out2F, out3F, out4F, out5F
        #  are     F2,    F3,    F4,    F5
        out2F, out3F, out4F, out5F = self.squeeze2F(torch.cat((out2sf, out2df), 1)), self.squeeze3F(out3), self.squeeze4F(out4), self.squeeze5F(out5)
        #                                 128-64                                 256-64               512-64     2048-64
        # Decoder-F(FPN-PAM)
        # output: outF is F
        outF = self.decoderF(out2F, out3F, out4F, out5F)

        shape = x.size()[2:] if shape is None else shape  # todo noresize
        out = F.interpolate(self.linearr2(outF), size=shape, mode='bilinear')
        return out, GS, GS3, GS4, GD, GD3, GD4
